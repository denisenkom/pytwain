...
class MainFrame(wx.Frame, TwainBase):
    ...
    def __init__(self, parent, id, title):
        wx.Frame.__init__(
            self, parent, id, title, wx.DefaultPosition, wx.Size(400, 300)
        )
        ...
        menu.Append(
            ID_ACQUIRE_NATIVELY,
            "Acquire &Natively",
            "Acquire an Image using Native Transfer Interface",
        )
        menu.Append(
            ID_ACQUIRE_BY_FILE,
            "Acquire By &File",
            "Acquire an Image using File Transfer Interface",
        )
    ...
    def MnuOpenScanner(self, event):
        self.OpenScanner(
            self.GetHandle(),
            ProductName="Simple wxPython Demo",
