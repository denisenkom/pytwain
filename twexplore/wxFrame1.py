...
[
    wxID_WXFRAME1FILEMENUITEMS0,
    wxID_WXFRAME1FILEMENUITEMS1,
    wxID_WXFRAME1FILEMENUITEMS2,
    wxID_WXFRAME1FILEMENUITEMS3,
    wxID_WXFRAME1FILEMENUITEMS4,
...
class wxFrame1(wx.Frame):
    def _init_coll_FileMenu_Items(self, parent):
        parent.Append(
            wxID_WXFRAME1FILEMENUITEMS0,
            "Connect",
            "Connect to Scanner or other TWAIN device",
        )
        parent.Append(
            wxID_WXFRAME1FILEMENUITEMS1,
            "Disconnect",
            "Disconnect From Scanner or TWAIN Device",
        )
        ...
        self.Bind(
            wx.EVT_MENU, self.OnFilemenuitems0Menu, id=wxID_WXFRAME1FILEMENUITEMS0
        )
        self.Bind(
            wx.EVT_MENU, self.OnFilemenuitems1Menu, id=wxID_WXFRAME1FILEMENUITEMS1
        )
        self.Bind(
            wx.EVT_MENU, self.OnFilemenuitems2Menu, id=wxID_WXFRAME1FILEMENUITEMS2
        )
        self.Bind(
            wx.EVT_MENU, self.OnFilemenuitems3Menu, id=wxID_WXFRAME1FILEMENUITEMS3
        )
        self.Bind(
            wx.EVT_MENU, self.OnFilemenuitems4Menu, id=wxID_WXFRAME1FILEMENUITEMS4
        )
    ...
    def _init_coll_listCaps_Columns(self, parent):
        parent.InsertColumn(
            col=0, format=wx.LIST_FORMAT_LEFT, heading="Capability", width=200
        )
        parent.InsertColumn(
            col=1, format=wx.LIST_FORMAT_LEFT, heading="Supported Values", width=400
        )
        parent.InsertColumn(
            col=2, format=wx.LIST_FORMAT_LEFT, heading="Current Value", width=100
        )
    ...
    def _init_ctrls(self, prnt):
        wx.Frame.__init__(
            ...
            parent=prnt,
            pos=wx.Point(254, 66),
            size=wx.Size(768, 537),
            style=wx.DEFAULT_FRAME_STYLE,
            title="TWAIN Explorer",
        )
        self._init_utils()
        ...
        self.splitterWindow1 = wx.SplitterWindow(
            ...
            parent=self,
            pos=wx.Point(128, -32),
            size=wx.Size(760, 491),
            style=wx.SP_3D,
        )

        self.txtLog = wx.TextCtrl(
            id=wxID_WXFRAME1TXTLOG,
            name="txtLog",
        ...
        self.listCaps = wx.ListCtrl(
            id=wxID_WXFRAME1LISTCAPS,
            name="listCaps",
            parent=self.splitterWindow1,
            pos=wx.Point(2, 2),
            size=wx.Size(756, 378),
            style=wx.LC_REPORT,
        ...
        self.listCaps.Bind(
            ...
            self.OnListcapsListItemSelected,
            id=wxID_WXFRAME1LISTCAPS,
        )
        ...
        self.statusBar1 = wx.StatusBar(
            id=wxID_WXFRAME1STATUSBAR1, name="statusBar1", parent=self, style=0
        )
    ...
    def DisplayException(self, exc, Title=None):
        ...
        dlg = wx.MessageDialog(self, txt, Title, wx.OK | wx.ICON_INFORMATION)
        dlg.ShowModal()
        dlg.Destroy()
    ...
    def OnFilemenuitems3Menu(self, event):
        ...
        if not hasattr(self, "CapabilityNames"):
            ...
            capnames = sorted(
                filter(lambda x: len(x) > 3 and x[0:3] == "CAP", twain.__dict__.keys())
            )
            capnames1 = sorted(
                filter(lambda x: len(x) > 4 and x[0:4] == "ICAP", twain.__dict__.keys())
            )
            capnames = capnames + capnames1
            capnames1 = sorted(
                filter(lambda x: len(x) > 4 and x[0:4] == "ACAP", twain.__dict__.keys())
            )
            self.CapabilityNames = capnames + capnames1

            ## I use these for selecting the default value to set a capability.
            typenames = filter(
                lambda x: len(x) > 4 and x[0:4] == "TWTY", twain.__dict__.keys()
            )
            self.typeIds = {}
        ...
        if (
            not hasattr(self, "LastSource")
            or self.LastSource != self.SS.GetSourceName()
        ):
